...
### [Display-Cutout Fullscreen Patch](patches/1-fullscreen.lua)

**Platforms:** Android 9 and later; tested on Samsung Galaxy Z Fold 8

Lets KOReader use the entire screen on phones and tablets with a camera cutout,
notch, or punch-hole camera. KOReader normally excludes the cutout's safe inset,
which can leave a full-width unused strip along the top of the display.

The patch enables Android's display-cutout layout mode and sizes KOReader's
framebuffer to the complete drawing surface. It uses the early `1-` patch prefix
because the corrected dimensions must be available before KOReader creates its
framebuffer.
...
### [Fold 8 Fix](patches/2-fold-8-fix.lua)
...
It also keeps text and UI elements at the same physical size when switching
between the cover and inner displays. The unfolded screen still gains the full
additional layout area, without KOReader enlarging everything to match its
greater pixel dimensions.
