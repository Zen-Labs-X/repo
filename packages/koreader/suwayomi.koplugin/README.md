...
The confirmation shows the captured download count and, when the 50-download limit applies, how many more chapters are available. Acceptance rechecks only the captured chapters, so changes can reduce the number queued without filling the batch with different chapters. If the chapter view or filter changes, run the action again. Successful actions update the chapter list and Downloads screen without a result popup. Failed or unconfirmed saves still show an error. Canceled or rejected actions preserve selection.
...
With **Delete after manual mark-read** enabled, single, selected, and previous-chapter mark-read actions save read state and accepted removal requests before removing only the captured CBZ. Reading metadata, backups, and directories are retained. Successful read and delete actions do not show result popups. Chapter rows show the current read/download state and outstanding removal problems, not receipts for completed removals. Busy or blocked removal requests and failed saves still show an explanation.
...
Blocked identity or containment checks preserve the file. No old cleanup record is assigned to a later replacement, even at the same pathname with identical contents. A fresh manual action can authorize a currently verified archive. Filesystems without reliable file identity remain blocked; concurrent external file writers, hot reload, and multiple writable KOReader processes are outside the safety guarantee. Ordinary **Delete** keeps its separate metadata-removal policy. Device results and the maintainer's accepted verification limits are recorded in [#37](https://github.com/LK4D4/suwayomi.koplugin/issues/37).

Routine cleanup retries, successful archive verification, and read-sync startup do not interrupt reading. Automatic read-sync startup failures retry quietly; an explicit **Sync** action still reports a startup failure. Cleanup warnings remain for unsafe files and unsupported saved data.

For diagnostics, create `suwayomi_debug.lua` in KOReader's settings directory containing `return { enabled = true }`, then restart KOReader. Download admission and manual read outcomes use the existing redacted key-value logs, including `suwayomi_debug.log` in that directory. Logging is off by default; no notification setting is required. Remove the debug configuration and restart KOReader to disable it.
