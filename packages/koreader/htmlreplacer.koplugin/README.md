...
1. **Add Rules**: Define regex patterns specific to this book
2. **Quick Apply**: Use "Quick Apply (Process + Apply)" to process and apply in one step
   - No preview
   - Automatically backs up original
   - Immediately replaces EPUB
   - Faster for confident changes
3. **Revert** (if needed): Use "Revert to Original" to restore from backup
...
### Preview Method (Recommended for first-time use)
...
### Quick Method (For confident changes)
1. **Open an EPUB** in KOReader
2. **Access the menu**: Tap menu → Style → Content tweaks
3. **Add New Rule** to create patterns
4. **Quick Apply (Process + Apply)** - Done! No preview step
...
- **Delimiter**: The footnote indicator (1, 2, *, **, †, ‡, §, etc.)
...
- `30000` = Add footnote only if 30000+ characters since the last one (default - about once per chapter)
- `2000` = Add footnote only if 2000+ characters since the last one (more frequent)
